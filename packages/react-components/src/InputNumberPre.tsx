...
import { dropZero } from '@eco/eco-utils/utils';
...
function getRegex (isDecimal: boolean, precision: number): RegExp {
  const decimal = '.';

  return new RegExp(
    isDecimal
      ? `^(0|[1-9]\\d*)(\\${decimal}\\d{0,${precision}})?$`
      : '^(0|[1-9]\\d*)$'
  );
}
...
function InputNumber ({ autoFocus, bitLength = DEFAULT_BITLENGTH, children, className = '', defaultValue, help, isDecimal, isFull, isSi, isDisabled, isError = false, isWarning, isZeroable = true, label, labelExtra, maxLength, maxValue, onChange, onEnter, onEscape, placeholder, value: propsValue, precision, npower = 0 }: Props): React.ReactElement<Props> {
  ...
  const [RRalue, updateRRvalue] = useState<string>('0');

  console.log('this is pre');
  useEffect((): void => {
    const _val = new Decimal(new Decimal(value || 0)).mul(new Decimal(10).pow(npower));
    const _bnv = new BN(_val.toString());

    updateRRvalue(value);
    onChange && onChange(_bnv);
  }, [onChange, value, npower]);

  useEffect(() => {
    updateRRvalue((_val) => {
      return dropZero(new Decimal(_val || 0).toFixed(precision || 0));
    });
  }, [precision]);
  ...
  const _onKeyDown = useCallback(
    (event: React.KeyboardEvent<Element>): void => {
      ...
      if (event.key.length === 1 && !isPreKeyDown) {
        ...
        if (!getRegex(isDecimal || !!si, precision).test(newValue)) {
          event.preventDefault();
        }
      }
    },
    [isDecimal, isPreKeyDown, si, precision]
  );
  ...
  const maxValueLength = getGlobalMaxValue(bitLength + precision * 8).toString().length - 1;
