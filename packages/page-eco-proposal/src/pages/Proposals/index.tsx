...
import { getMembers } from '@eco/eco-utils/utils';
// const Members = [
...
const PageProposals = () => {
  ...
  return (
    <div>
      <Part>
        <Panel title={t<string>('资产审查委员会介绍')}>
          <div>
            {t<string>('ECO2 Ledger的资产审查委员会，负责碳汇资产进出ECO2 Ledger的审查，确保链上碳汇资产的真实性和避免重复计算的问题，审查中会仔细核查申请上链的碳汇资产、 在信息、数量和相关报告中，是否与申请资料吻合。当碳汇要进入或离开ECO2 Ledger都必须经由资产审查委员会审查。资产审查委员会由五位碳汇行业之专业人士，经由节点投票后行使职权，凡链上碳汇资产之进出皆须经资产审查委员会当中三位以上同意后执行。')}
          </div>
        </Panel>
        <Panel title={t<string>('成员列表')}>
          <MembersRow>
            {Members.map((member) => {
